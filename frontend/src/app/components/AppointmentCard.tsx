...
import { Clock, User, Scissors, Calendar, Phone, MessageCircle } from 'lucide-react';
import { WhatsAppPreviewDialog } from '@/app/components/ui/whatsapp-preview-dialog';
    ...
    <>
      <Card className="bg-white dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/70 transition-colors">
        <CardContent className="p-4">
          ...
          {showWhatsApp && cita.estado !== 'completada' && (
            <div className="mt-3">
              <button
                onClick={() => setWaOpen(true)}
                className="w-full px-3 py-1.5 text-xs bg-green-500/15 text-green-600 dark:text-green-400 rounded hover:bg-green-500/25 transition-colors flex items-center justify-center gap-2"
              >
            ...
            <div className="mt-2 flex gap-2">
              {cita.estado === 'pendiente' && (
                <button
                  ...
                  className="flex-1 px-3 py-1.5 text-xs bg-blue-500/20 text-blue-500 dark:text-blue-400 rounded hover:bg-blue-500/30 transition-colors"
                >
                  Confirmar
                </button>
              )}
              <button
                ...
                className="flex-1 px-3 py-1.5 text-xs bg-green-500/20 text-green-500 dark:text-green-400 rounded hover:bg-green-500/30 transition-colors"
              >
                Completar
              </button>
              <button
                onClick={() => onStatusChange(cita.id, 'cancelada')}
                className="flex-1 px-3 py-1.5 text-xs bg-red-500/20 text-red-500 dark:text-red-400 rounded hover:bg-red-500/30 transition-colors"
              >
                Cancelar
              </button>
            </div>
